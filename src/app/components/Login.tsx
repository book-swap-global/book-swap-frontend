...
export default function Login() {
  ...
  const onSubmit = async (data: { mobile: string }) => {
    ...
    if (!phoneE164.startsWith("+")) {
      ...
      alert(
        "Please enter a valid phone number with country code (e.g. +8190  12345678)"
      );
      return;
    }
  ...
  return (
    <>
      <AuthCard>
        ...
        <p className="text-gray-700 text-xs font-medium mb-5 text-center">
          Enter your phone number to continue
        </p>
        <form onSubmit={handleSubmit(onSubmit)}>
          <label className="block text-xs text-gray-700 mb-2 font-sm">
            Phone Number
          </label>
          <div className="mb-4">
            <Controller
